...
int* rotLeft(int a_count, int* a, int d, int* result_count) {
    // save the index of the first element we looked at
    int first = 0;

    // set the current index to the first element we will look at
    int currIndex = first;

    // save the value of the current index
    int temp = a[currIndex];
    do {
        // set the next index to look at
        int nextIndex = (currIndex + d) % a_count;

        // set the new value of the current index to either a value being
        // shifted from a different position or the first element we saved
        a[currIndex] = (nextIndex == first) ? temp : a[nextIndex];

        // move the current index to the calculated next index
        currIndex = nextIndex;
    } while (currIndex != first);

    // result count will always be the same size as the array sent in
