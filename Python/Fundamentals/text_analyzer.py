...
def analyze_text(filename):
    ...
    Returns: A tuple where the first element is the number of lines in
        the file and the second is the number of characters.
    """
    lines = 0
    characters = 0
...
class TextAnalysisTests(unittest.TestCase):
    ...
    def setUp(self):
        """Fixure that creates a file for the text methods to use."""
        self.filename = 'textAnalysisTestFile.txt'
        with open(self.filename, 'w') as tmpFile:
            tmpFile.write('Now we are engaged in a great civil war.\n'
                          'testing whether that nation,\n'
                          'or any nation so conceived and so dedicated,\n'
                          'can long endure.')

    def tearDown(self):
        """Fixture that deletes the files ued by the test methods."""
        try:
    ...
    def test_function_runs(self):
        """Base smoke test: does the function run."""
    ...
    def test_character_count(self):
        """Check that the character count ia correct."""
